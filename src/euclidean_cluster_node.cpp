...
: Node("euclidean_cluster_node", options)
{
  ...
  const int max_cluster_size = this->declare_parameter("max_cluster_size", 1000);
  const double distance_threshhold_seg = this->declare_parameter("distance_threshhold_seg", 0.1);
  const float tolerance = this->declare_parameter("tolerance", 0.1);
  cluster_ =
    std::make_shared<EuclideanCluster>(use_height, min_cluster_size, max_cluster_size, distance_threshhold_seg, tolerance);
  ...
  clustered_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>("clustered_points", rclcpp::QoS{1});
}
